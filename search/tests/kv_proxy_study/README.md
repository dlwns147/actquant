...
05_plot.py             Renders all final figures from data/.
06_measure_prefill_stride.py
                       L_prefill_stride sweep (CE + JSD).
data/
  fp16_logits/             Per-sample [A,V] fp16 logits (~32 MB each).
  fp16_baseline.json       Per-sample L_fp16 (CE).
...
figures/
  ...
  prefill_stride_bias.png   Δ vs answer-stride (CE row + JSD row).
```
...
CUDA_VISIBLE_DEVICES=0 python tests/kv_proxy_study/01_dump_fp16.py \
...
for SEQ in 2048 4096 8192; do
  for CFG in 2:2 2:3 2:4 3:2 3:3 3:4 4:2 4:3 4:4; do
    CUDA_VISIBLE_DEVICES=0 python tests/kv_proxy_study/02_measure_ce.py \
      --model_path /SSD/huggingface/meta-llama/Llama-3.1-8B-Instruct \
...
for SEQ in 2048 4096 8192; do
  for CFG in 2:2 2:3 2:4 3:2 3:3 3:4 4:2 4:3 4:4; do
    CUDA_VISIBLE_DEVICES=0 python tests/kv_proxy_study/03_measure_jsd.py \
      --model_path /SSD/huggingface/meta-llama/Llama-3.1-8B-Instruct \
      --residual_length 128 --strides 128,256,512,1024 \
      --n_samples 4 --prompt_len $SEQ --answer_len 128 --configs $CFG
  done
...
for SEQ in 2048 4096 8192; do
  for CFG in 2:2 2:3 2:4 3:2 3:3 3:4 4:2 4:3 4:4; do
    CUDA_VISIBLE_DEVICES=0 python tests/kv_proxy_study/06_measure_prefill_stride.py \
      --model_path /SSD/huggingface/meta-llama/Llama-3.1-8B-Instruct \
...
for SEQ in 2048 4096 8192; do
  CUDA_VISIBLE_DEVICES=0 python tests/kv_proxy_study/04_measure_speed.py \
    --model_path /SSD/huggingface/meta-llama/Llama-3.1-8B-Instruct \
    --prompt_len $SEQ --answer_len 128 \
    --residual_length 128 --strides 128,256,512,1024 --kbits 2 --vbits 2
done

# 6. Render all final figures
python tests/kv_proxy_study/05_plot.py
```
