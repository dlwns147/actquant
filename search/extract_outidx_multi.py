...
def extract_outlieridx(model, dataloader, dev, args):
    ...
    dirname = os.path.join(args.output_dir, f"w{args.wbits}_r{'_'.join(map(str, args.target_rank))}_{args.dataset}")
    ...
    outlier = {'n_outlier': args.target_rank}
    for i in range(len(layers)):
        ...
        for names in sequential:
            ...
            gptq_owq = {}
            for name in subset:
                gptq_owq[name] = GPTQ_OWQ(subset[name], n_out=n_out_dict[name][0])
            ...
            for name in meta['sequential'][0] + meta['sequential'][2] + meta['sequential'][3]:
                for n_out in n_out_dict[name]:
                    ...
                    out_ids_list[name][n_out] = out_ids
            ...
            # for name in subset: # all
            for name in meta['sequential'][3]: # down
                print(f"Quantizing {meta['prefix']}.{i}.{name}")
                
                # torch.save(out_ids, os.path.join(dirname, f"{meta['prefix']}.{i}.{name}.pth"))
                # outlier[f"{meta['prefix']}.{i}.{name}"] = [out_ids, W[out_ids]]
                # outlier[f"{meta['prefix']}.{i}.{name}"] = out_ids.tolist()
                outlier[f"{meta['prefix']}.{i}.{name}"] = {}
                for n_out in n_out_dict[name]:
                    # outlier[f"{meta['prefix']}.{i}.{name}"][n_out] = gptq_owq[name].quantizer.out_ids.tolist()
                    outlier[f"{meta['prefix']}.{i}.{name}"][n_out] = out_ids_list[name][n_out].tolist()
                gptq_owq[name].free()
                W = subset[name].weight.data
                key = f"{meta['prefix']}.{i}.{name}"
                print(f"""name : {name}, out_ids : {len(outlier[key])}, W : {W.shape}, W[out_ids] : {[W[:, out_ids].shape for out_ids in outlier[key].values()]}""")
    ...
    # out_ids = sorted(torch.topk(sensitivity_sum, args.target_rank).indices.cpu().tolist())
    ...
    for i in range(len(layers)):
        ...
        for names in sequential:
            ...
            for name in meta['sequential'][0] + meta['sequential'][2]:
                outlier[f"{meta['prefix']}.{i}.{name}"] = {}
                for n_out in n_out_dict[name]:
                    out_ids = sorted(torch.topk(sensitivity_sum, n_out).indices.cpu().tolist())
                    # outlier[f"{meta['prefix']}.{i}.{name}"] = [out_ids, W[out_ids]]
                    outlier[f"{meta['prefix']}.{i}.{name}"][n_out] = out_ids
                W = subset[name].weight.data
                key = f"{meta['prefix']}.{i}.{name}"
                print(f"""name : {name}, out_ids : {len(outlier[key])}, W : {W.shape}, W[out_ids] : {[W[:, out_ids].shape for out_ids in outlier[key]]}""")
                
    output_path = os.path.join(dirname, f'outlier.pth')
    print(f'outlier: ')
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--target_rank', type=int, nargs='+', default=[],
