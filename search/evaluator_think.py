...
class LlamaEvaluatorThink:
    ...
    def remove_linears(self, model, config):
        for blk_idx in range(int(config['n_block'])):
            for linear_group in config['linear']:
                for linear in linear_group.split(','):
                    delsubattr(getblock(model, config)[blk_idx], linear)
    ...
    def sample(self, arch):
        ...
        # Set KV cache quantization bits
        if self.method['kv'] == 'hqq':
            ...
            # Set pruning ratio for ThinK
            if 'pruning_ratio' in arch:
                # Note: ThinK uses pruning_ratio as retention rate (0.25 = 25% retention = 75% pruning)
                # But the config might expect it differently, so we need to check the actual implementation
                # For now, we'll set it per layer
                if hasattr(self.model.config, 'pruning_ratio'):
                    # If it's a single value, use the first one (assuming all layers use the same)
                    if isinstance(arch['pruning_ratio'], list):
                        pruning_ratio_val = arch['pruning_ratio'][0] if len(arch['pruning_ratio']) > 0 else 1.0
                    else:
                        pruning_ratio_val = arch['pruning_ratio']
                    self.model.config.pruning_ratio = pruning_ratio_val
                    
                    # Also set for each layer if needed
                    for layer_idx in range(int(self.config['n_block'])):
                        layer = getblock(self.model, self.config)[layer_idx]
                        if hasattr(layer, 'self_attn') and hasattr(layer.self_attn, 'pruning_ratio'):
                            layer_pruning_ratio = arch['pruning_ratio'][layer_idx] if isinstance(arch['pruning_ratio'], list) and layer_idx < len(arch['pruning_ratio']) else pruning_ratio_val
                            layer.self_attn.pruning_ratio = layer_pruning_ratio
    ...
    def eval(self, accelerator, arch, metric, model=None, loss_func='cross_entropy'):
        ...
        for dataset, loader in loaders.items():
            result = eval_metric(
                ...
                dense_logits=self.dense_logits.get(dataset, None),
                key_token_list=self.key_token_list.get(dataset, None),
                loss_func=loss_func)
