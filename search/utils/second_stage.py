...
def _select_blocks(j, c, encode_fn, eps, eps_rel, div_k, seed):
    ...
    return f, idx, blocks[di], c[idx][di]
...
def load_block_pools(w_expr, kv_expr, ss, w_eps=0.0, kv_eps=0.0, eps_rel=0.0,
    ...
    relative `eps_rel`) → structural-diversity prune (data-driven, no random)."""
    nw = nw_split(ss)
...
class BandTable:
    ...
    def _fit(G, cc, xu_part, edges, alpha):
        ...
        for b in range(B):
            Gb = G[(cc > edges[b]) & (cc <= edges[b + 1])]
            for ci in range(D):
                cnt = np.bincount(Gb[:, ci], minlength=MO).astype(float)
                S[b, ci] = int(cnt.argmax()) if len(Gb) else int(xu_part[ci])
                cnt = np.where(valid[ci], cnt + alpha, 0.0)      # smooth valid options only
                P[b, ci] = cnt / cnt.sum()
        return edges, P, S
    ...
    def draw(self, g_idx, band_w, band_kv):
        ...
        p = self.Pw[band_w, g_idx] if g_idx < self.nw else self.Pk[band_kv, g_idx - self.nw]
        return int(min(np.searchsorted(np.cumsum(p), np.random.random()), len(p) - 1))

    def stair(self, t_w, t_kv):
        """P2 deterministic staircase decode: band-consensus genome at target comps."""
        return np.concatenate([self.Sw[int(self.band(t_w, 'w'))],
                               self.Sk[int(self.band(t_kv, 'kv'))]])

    def sample_genome(self, t_w, t_kv):
        """stochastic staircase: every cell drawn from its band distribution (diverse
        in-band seed; same table as P1 → argmax=stair / sample=this)."""
        bw = int(self.band(t_w, 'w')); bk = int(self.band(t_kv, 'kv'))
        cw = np.cumsum(self.Pw[bw], axis=1); ck = np.cumsum(self.Pk[bk], axis=1)
        rw = np.random.random(cw.shape[0])[:, None]; rk = np.random.random(ck.shape[0])[:, None]
        gw = np.minimum((cw < rw).sum(1), cw.shape[1] - 1)
        gk = np.minimum((ck < rk).sum(1), ck.shape[1] - 1)
        return np.concatenate([gw, gk])
...
class KnowledgeMutation(Mutation):
    ...
    def _do(self, problem, X, **kw):
        ...
        bw = bk = None
        if self.bt is not None and self.comp is not None:            # P1: per-indiv own-axis bands
            C = self.comp.batch(np.clip(np.round(X), 0, self.xu).astype(int), self.comp_obj)
        ...
        for i in range(len(X)):
            ...
            for g in np.where(np.random.random(nv) < self.base * self.w)[0]:
                if np.random.random() < self.p_val:                  # 1st-stage-plausible value
                    if bw is not None:                               # band-conditional draw (P1)
                        X[i, g] = self.bt.draw(g, int(bw[i]), int(bk[i]))
                    else:                                            # legacy global column draw
...
class ArchFeatures:
    ...
    def __init__(self, ss, comp):
        ...
        self.w_lut = []
        for linear in ss.config['linear']:
            opts = getattr(ss, f"{linear.split('.')[-1]}_option")
            self.w_lut.append(np.array([o[0] if isinstance(o, (list, tuple)) else o
                                        for o in opts], float))
        self.k_bits = np.array([o[0] for o in ss.k_option], float)
        self.v_bits = np.array([o[0] for o in ss.v_option], float)
        self.k_lgs = np.log2([float(o[1]) for o in ss.k_option])
        self.v_lgs = np.log2([float(o[1]) for o in ss.v_option])
        self.k_pr = np.array(ss.k_pruning_dim_option, float)
        self.v_pr = np.array(ss.v_pruning_dim_option, float)

    def __call__(self, X):
        X = np.asarray(X, float)
        X = np.clip(np.round(X), 0, None).astype(int)
        Xr = X.reshape(len(X), self.nl + 4, self.nb)
        cols = [self.w_lut[m][Xr[:, m, :]].mean(1) for m in range(self.nl)]
        cols += [self.k_bits[Xr[:, self.nl]].mean(1),  self.v_bits[Xr[:, self.nl + 1]].mean(1),
