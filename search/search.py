...
class Search:
    def __init__(self, config, accelerator, device_map, kwargs):
        ...
        self.comp_obj = kwargs.pop('comp_obj', ['wbits', 'kbits', 'vbits'])  # second objective to optimize simultaneously
        self.comp_obj_min = kwargs.pop('comp_obj_min', [min(w_bits), min(k_bits), min(v_bits)])
        self.comp_obj_max = kwargs.pop('comp_obj_max', [max(w_bits), max(k_bits), max(v_bits)])
        ...
        # self.sensitivity_metric = kwargs.pop('sensitivity_metric' , 'loss')
        ...
        pass_module = {'w': [], 'k': [], 'v': []}
        # pass_module = {}
        # if 'wbits' in self.comp_obj:
        #     pass_module['w'] = []
        # if 'kvbits' in self.comp_obj:
        #     pass_module['k'] = []
        #     pass_module['v'] = []
        # else:
        #     if 'kbits' in self.comp_obj:
        #         pass_module['k'] = []
        #     if 'vbits' in self.comp_obj:
        #         pass_module['v'] = []
        ...
        if self.sensitivity_result_path:
            ...
            print(f'upper_bound: {np.median(total_sensitivity_list) * 2}')
            pass_idx_list = np.where(total_sensitivity_list > np.median(total_sensitivity_list) * 2)[0].tolist()
        ...
        # pass_w_linear_list = kwargs.pop('pass_w_linear', [])
        # # sensitive_a_linear_list = kwargs.pop('pass_act_linear_list', [])
        # pass_k_layer_list = kwargs.pop('pass_k_layer', [])
        # pass_v_layer_list = kwargs.pop('pass_v_layer', [])        
        # pass_module = {'w': pass_w_linear_list, 'k': pass_k_layer_list, 'v': pass_v_layer_list}
        # self.pass_module = pass_module
...
if __name__ == '__main__':
    ...
    parser.add_argument('--comp_obj', type=str, nargs='+', default=['wbits', 'kbits', 'vbits'], choices=['wbits', 'kvbits', 'kbits', 'vbits'], 
                        help='complexity objectives to optimize simultaneously')
    parser.add_argument('--comp_obj_min', type=float, nargs='+', default=[2, 2, 2], 
                        help='')
    parser.add_argument('--comp_obj_max', type=float, nargs='+', default=[4, 4, 4], 
                        help='')
