...
gsm8k_loader = get_loader('gsm8k', n_sample=32, train=True, seed=0, seqlen=256, min_seqlen=192, batch_size=8, tokenizer=tokenizer)
# gsm8k_loader = get_loader('gsm8k', n_sample=1, train=True, seed=0, seqlen=256, min_seqlen=256, batch_size=8, tokenizer=tokenizer)
...
with torch.no_grad():
    for inputs, attention_mask, labels in gsm8k_loader:
        ...
        logits = model(inputs.cuda(), attention_mask=attention_mask.cuda()).logits
        # import pdb; pdb.set_trace()
        break
