...
def get_wikitext2_trainenc(seed, nsamples, seqlen, model, hf_token=None):
...
def get_c4_trainenc(seed, nsamples, seqlen, model, hf_token=None):
    if hf_token is None:
        tokenizer = transformers.AutoTokenizer.from_pretrained(model, use_fast=False)
    else:
        tokenizer = transformers.AutoTokenizer.from_pretrained(model, use_fast=False, use_auth_token=hf_token)
    ...
    valdata = datasets.load_dataset('allenai/c4', data_files={'validation': 'en/c4-validation.00000-of-00008.json.gz'}, split='validation')
    traindata = traindata.shuffle(seed=seed)
...
def get_train_loaders(
    ...
    if 'wikitext2' in name:
        return get_wikitext2_trainenc(nsamples=nsamples, seed=seed, seqlen=seqlen, model=model, hf_token=hf_token)
    if 'c4' in name:
        return get_c4_trainenc(nsamples=nsamples, seed=seed, seqlen=seqlen, model=model, hf_token=hf_token)