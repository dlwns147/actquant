...
def main():
    ...
    for dataset in args.eval_datasets:
        ...
        dataset_ppl = eval_utils.evaluator(model, testloader, utils.DEV, dataset, args, args.eval_layerwise)
    ...
    if not args.lm_eval:
        ...
        metric_vals = {task: result.get('acc_norm,none', result['acc,none']) for task, result in results.items()}
        metric_vals['acc_avg'] = round(sum(metric_vals.values()) / len(metric_vals.values()), 4)
        print(metric_vals)
        print(list(metric_vals.keys()))
        print(list(metric_vals.values()))
