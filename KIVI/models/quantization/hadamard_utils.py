...
def matmul_hadU_cuda(X, hadK, K):
    ...
    input = hadK.to(input.dtype) @ input
...
def apply_exact_had_to_linear(module, had_dim=-1, output=False):
    ...
    if had_dim == -1:
        ...
        if output:
            ...
            # n = W_.shape[1]
            # W_ = fast_hadamard_transform.hadamard_transform(W_.reshape(-1, n//had_dim, had_dim), 
            #                                                 scale=1/math.sqrt(had_dim)).reshape(init_shape)
            
            W_ = fast_hadamard_transform.hadamard_transform(
                W_.reshape(-1, init_shape[-1]//had_dim, had_dim).transpose(1, 2), 
                scale=1/math.sqrt(init_shape[-1]//had_dim)
                ).transpose(1, 2).reshape(init_shape)
