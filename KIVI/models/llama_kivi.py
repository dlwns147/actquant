...
class LlamaFlashAttention_KIVI(LlamaAttention_KIVI):
    def forward(
        ...
        **kwargs,
        ...
        query_states = query_states.view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2) # B, H, T, D
        ...
        if self.quant_kv_output:
            key_states = fake_quant(key_states, self.group_size, self.k_bits, self.k_quant_per)
            value_states = fake_quant(value_states, self.group_size, self.v_bits, self.v_quant_per)
...
class LlamaDecoderLayer_KIVI(nn.Module):
    def __init__(self, config: LlamaConfig):
        ...
        self.self_attn = (
            ...
            LlamaFlashAttention_KIVI(config=config)
        )
