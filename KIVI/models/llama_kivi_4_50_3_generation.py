...
from models.KIVICache import KIVIDynamicCache, KIVICacheConfig

logger = logging.get_logger(__name__)


def convert_generation_kivi(kivi_cache_config: KIVICacheConfig):
    def _prepare_cache_for_generation(
        ...
        if generation_config.cache_implementation is not None:
            if generation_config.cache_implementation in NEED_SETUP_CACHE_CLASSES_MAPPING:
                ...
                model_kwargs[cache_name] = KIVIDynamicCache(kivi_cache_config)

        # Use DynamicCache() instance by default. This will avoid back and forth from legacy format that
        # keeps copying the cache thus using much more memory
        else:
            model_kwargs[cache_name] = (
                KIVIDynamicCache(kivi_cache_config)
                if not requires_cross_attention_cache
                else EncoderDecoderCache(KIVIDynamicCache(kivi_cache_config), KIVIDynamicCache(kivi_cache_config))
            )
