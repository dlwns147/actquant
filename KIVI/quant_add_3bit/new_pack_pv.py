...
@triton.jit
def _pack_along_last_dim_3bit_old(
	...
	# 3bit -> 10 values (30 bits) + spill logic for MSB/LSB across words
	num_int_per_3y_dim = num_feats // 32 * 3		## 32 is INT32 and 3 is 3bit
	bid = tl.program_id(axis=0)
	## 아마 3개 단위로 묶어서 처리, 만약 num_feat가 128(residual length)이라면, 4개의 yid가 생성됨. (96bit(32 * 3)를 한 번에 처리하므로 32개의 element를 처리하므로 4개의 yid가 생성)
	yid = tl.program_id(axis=1)
	offs_N = bid * BLOCK_SIZE_N + tl.arange(0, BLOCK_SIZE_N)
	block_start = intensor_ptr + offs_N * num_feats + yid * 32 # offset of the first element at current tile
	# packed = tl.zeros((BLOCK_SIZE_N, 3), dtype=tl.int32)
	pack_0 = tl.zeros((BLOCK_SIZE_N,), dtype=tl.int32)
	pack_1 = tl.zeros((BLOCK_SIZE_N,), dtype=tl.int32)
	pack_2 = tl.zeros((BLOCK_SIZE_N,), dtype=tl.int32)

	i = 0
	for j in range(i, i + 10):
		ptr = block_start + j
		element = tl.load(ptr, mask=offs_N<N, other=0) & 0x7
		element = element << (3 * (j - i))
		# Combine the value using bitwise OR
		pack_0 = pack_0 | element

	i += 10
	ptr = block_start + i
	element = tl.load(ptr, mask=offs_N<N, other=0) & 0x7
	pack_0 = pack_0 | (element << 30)
	pack_1 = pack_1 | ((element >> 2) & 1)

	i += 1
	for j in range(i, i + 10):
		ptr = block_start + j
		element = tl.load(ptr, mask=offs_N<N, other=0) & 0x7
		element = element << (3 * (j - i) + 1)
		# Combine the value using bitwise OR
		pack_1 = pack_1 | element

	i += 10
	ptr = block_start + i
	element = tl.load(ptr, mask=offs_N<N, other=0) & 0x7
	pack_1 = pack_1 | (element << 31)
	pack_2 = pack_2 | ((element >> 1) & 0x3)

	i += 1
	for j in range(i, i + 10):
		ptr = block_start + j
		element = tl.load(ptr, mask=offs_N<N, other=0) & 0x7
		element = element << (3 * (j - i) + 2)
		# Combine the value using bitwise OR
		pack_2 = pack_2 | element

	tl.store(code_ptr + offs_N * num_int_per_3y_dim + 3 * yid, pack_0, mask=offs_N < N)
	tl.store(code_ptr + offs_N * num_int_per_3y_dim + 3 * yid + 1, pack_1, mask=offs_N < N)
	tl.store(code_ptr + offs_N * num_int_per_3y_dim + 3 * yid + 2, pack_2, mask=offs_N < N)
...
def _pack_along_last_dim_3bit(
	...
	for j in tl.static_range(10):
		ptr = base + j
		element = tl.load(ptr, mask=row_mask, other=0) & 0x7
		pack_0 |= element << (3 * j)
	...
	tl.store(code_ptr + offs_N * num_int_per_3y_dim + 3 * yid, pack_0, mask=row_mask)
	tl.store(code_ptr + offs_N * num_int_per_3y_dim + 3 * yid + 1, pack_1, mask=row_mask)
	tl.store(code_ptr + offs_N * num_int_per_3y_dim + 3 * yid + 2, pack_2, mask=row_mask)

# 최적화 버전 - 포인터 사전 계산 및 불필요한 연산 제거
@triton.jit
def _pack_along_last_dim_3bit_optimized(
	bits: tl.constexpr,		# 3bit
	intensor_ptr,
	code_ptr,		# N * (ceil(T /// 32 * 3)) -> T will be 128(Residual length of KIVI)
	N,
...
@triton.jit
def _pack_along_last_dim_3bit_old_old(
