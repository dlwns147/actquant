...
// 4비트면 하나의 int32에 8개의 값이 들어감. 이게 결국 128개의 column = OC이 16개의 packing된 column으로 나옴
// 여기서 Grid가 BS * (OC/pack_factor(8) + 3) / 4이 되는데, 값은 4가 됨.
// 왜? 왜 4를?

// 일단 하나의 thread가 pack_factor개의 값을 처리함. 즉 하나의 packed element를 가져와서 늘림.
// warp는 32개의 thread로 이루어져 있으므로, 32개의 packed element를 처리함.
// 이때 block은 4개의 warp로 이루어져 있으므로, 하나의 block은 총 128개의 packed element를 처리함.
// OC / pack_factor는 몇 개의 packed element인지 알려주는 값
...
__global__ void bgemv3_kernel_outer_dim(
  ...
  const int IC, const int OC, const int group_size, const int nh, const int nh_kv){
    ...
    int _batch_idx = batch_idx / ratio;
    // const uint32_t*  weight = _weight + _batch_idx * OC * IC / pack_factor;
    // const uint32_t*  weight = _weight + _batch_idx * OC * IC * 3 / 32;
    // const half* scaling_factors = _scale + _batch_idx * OC * IC / group_size;
    // const half* zeros = _zeros + _batch_idx * OC * IC / group_size;
    ...
    for (int k=0; k < (ICR + TILE_DIM - 1) / TILE_DIM; k++){
      ...
      for (int i=0; i<4; i++){
        if (weight_offset + i < OC * ICR * 3 / 32)
          qw[0][i] = *(weight + weight_offset + i);
        if (weight_offset + i + ICR < OC * ICR * 3 / 32)
          qw[1][i] = *(weight + weight_offset + i + ICR);
        if (weight_offset + i + 2 * ICR < OC * ICR * 3 / 32)
          qw[2][i] = *(weight + weight_offset + i + 2 * ICR);
        if (scale_mn_offset + i < OC * ICR / group_size){
          cscale[i] = *(scaling_factors + scale_mn_offset + i);
          czero[i] = *(zeros + scale_mn_offset + i);
        }
      ...
      for (int ic_0 = 0; ic_0 < 4; ic_0++){
        ...
        for (; ic_1 < 10; ic_1++){
          ...
          if (oc_idx < OC){
            ...
            cur_packed_weight_0 = cur_packed_weight_0 >> bit;
            psum[ic_1] += dequantized_weight * (double)cur_inp;
          }
        }
        ...
        dequantized_weight = (double)cur_scale * (double)cur_single_weight_fp + (double)cur_zero;
        cur_packed_weight_1 = cur_packed_weight_1 >> 1;
        psum[ic_1] += dequantized_weight * (double)cur_inp;
        ic_1++;
        ...
        for (; ic_1 < 21; ic_1++){
          ...
          if (oc_idx < OC){
            ...
            dequantized_weight = (double)cur_scale * (double)cur_single_weight_fp + (double)cur_zero;
            cur_packed_weight_1 = cur_packed_weight_1 >> bit;
            psum[ic_1] += dequantized_weight * (double)cur_inp;
          }
        }
        ...
        dequantized_weight = (double)cur_scale * (double)cur_single_weight_fp + (double)cur_zero;
        cur_packed_weight_2 = cur_packed_weight_2 >> 2;
        psum[ic_1] += dequantized_weight * (double)cur_inp;
        ic_1++;
        ...
        for (; ic_1 < 32; ic_1++){
          ...
          if (oc_idx < OC){
            ...
            dequantized_weight = (double)cur_scale * (double)cur_single_weight_fp + (double)cur_zero;
            cur_packed_weight_2 = cur_packed_weight_2 >> bit;
            psum[ic_1] += dequantized_weight * (double)cur_inp;
          }
        }
      }
