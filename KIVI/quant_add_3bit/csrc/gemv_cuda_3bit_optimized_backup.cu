...
__global__ void bgemv4_kernel_outer_dim(
  ...
  const int IC, const int OC, const int group_size, const int nh, const int nh_kv){
    ...
    const uint32_t*  weight = _weight + (size_t)_batch_idx * OC * IC / pack_factor;    // overflow 방지를 위해 size_t 형변환
    const half* scaling_factors = _scale + (size_t)_batch_idx * OC * IC / group_size;    // overflow 방지를 위해 size_t 형변환
    const half* zeros = _zeros + (size_t)_batch_idx * OC * IC / group_size;    // overflow 방지를 위해 size_t 형변환
...
/*
Computes Batched 3-bit GEMV (group_size = 64).
...
기존 대비 수정 사항:
double -> float
vectorized memory load(float4, float2)
shared memory load
조건문 분기 제거
__ldg(read only) 사용
Loop Unrolling
...
__global__ void bgemv3_kernel_outer_dim(
  ...
  const int IC, const int OC, const int group_size, const int nh, const int nh_kv){
    ...
    const int ratio = nh / nh_kv;
    int _batch_idx = batch_idx / ratio;
    const uint32_t*  weight = _weight + (size_t)_batch_idx * OC * IC * 3 / 32;    // overflow 방지를 위해 size_t 형변환
    const half* scaling_factors = _scale + (size_t)_batch_idx * OC * IC / group_size;    // overflow 방지를 위해 size_t 형변환
    const half* zeros = _zeros + (size_t)_batch_idx * OC * IC / group_size;    // overflow 방지를 위해 size_t 형변환
    const int TILE_DIM = 128;
    const int num = 0xFF >> (8-bit);
    ...
    for (int k=0; k < (ICR + TILE_DIM - 1) / TILE_DIM; k++){
      ...
      // Weight: float4 로드 (uint32_t × 4 = 16바이트)
      if (weight_offset + 3 < OC * ICR * 3 / 32) {
        *((float4*)qw[0]) = __ldg((float4*)(weight + weight_offset));
        *((float4*)qw[1]) = __ldg((float4*)(weight + weight_offset + ICR));
        *((float4*)qw[2]) = __ldg((float4*)(weight + weight_offset + 2 * ICR));
      }
      // Scale/Zero: float2 로드 (half × 4 = 8바이트)
      ...
      // Input: float2 로드 (half × 4 = 8바이트)
      // if (inputs_ptr_delta + 3 < ICR) {
      //   *((float2*)inp) = __ldg((float2*)(inputs + inputs_ptr_delta));
      // }
      
      // ===================== 기존 Scalar Load (주석 처리) =====================
      // for (int i=0; i<4; i++){
      //   if (weight_offset + i < OC * ICR * 3 / 32)
      //     qw[0][i] = __ldg(weight + weight_offset + i);
      //   if (weight_offset + i + ICR < OC * ICR * 3 / 32)
      //     qw[1][i] = __ldg(weight + weight_offset + i + ICR);
      //   if (weight_offset + i + 2 * ICR < OC * ICR * 3 / 32)
      //     qw[2][i] = __ldg(weight + weight_offset + i + 2 * ICR);
      //   if (scale_mn_offset + i < OC * ICR / group_size){
      //     cscale[i] = __ldg(scaling_factors + scale_mn_offset + i);
      //     czero[i] = __ldg(zeros + scale_mn_offset + i);
      //   }
      //   if (inputs_ptr_delta + i < ICR)
      //     inp[i] = __ldg(inputs + inputs_ptr_delta + i);
      // }

      float cur_single_weight_fp = 0.0f;
      // double dequantized_weight = 0.0;
      ...
      for (int ic_0 = 0; ic_0 < 4; ic_0++){
        ...
        for (; ic_1 < 10; ic_1++){
          ...
          // dequantized_weight = (double)cur_scale * (double)cur_single_weight_fp + (double)cur_zero;
